...
class CBXPetitionShortCodes
{
    ...
    public function cbxpetition_summary_display($atts)
    {
        ...
        $atts = shortcode_atts([
            'petition_id' => $petition_id,
            'sections'    => apply_filters('cbxpetition_summary_shortcode_default_sections',
                'title,content,stat,expire_date')
                ...
                if (in_array('expire_date', $sections)) {
                    ...
                    if ($expire_date == '') {
                        ...
                        if ($expire_date < $now_date) {
                            ...
                            $expire_info = sprintf(esc_html__('Sorry, petition already expired on %s', 'cbxpetition'),
                                $expire_date->format('Y-m-d H:i:s'));
                        } else {
                            $expire_info = $expire_date->format(apply_filters('cbxpetition_expire_date_formatted',
                                $date_time_format, $date_format, $time_format));
                        }
                    }
                    $output .= '<p class="cbxpetition_expire_wrapper">'.esc_html__('Expire Date',
                            'cbxpetition').' : '.$expire_info.'</p>';
                }
    ...
    public function cbxpetition_banner_display($atts)
    {
        ...
        return cbxpetition_get_template_html('petition/banner.php', [
            'petition_id'        => $petition_id,
    ...
    public function cbxpetition_stat_display($atts)
    {
        ...
        $style         = absint($style);
        $template_file = ($style < 2) ? 'petition/stat.php' : 'petition/stat-'.$style.'.php';
        ...
        return cbxpetition_get_template_html($template_file, [
                'petition_id'     => $petition_id,
                'target'          => $target,
                'signature_count' => $signature_count,
                'signature_ratio' => $signature_ratio,
