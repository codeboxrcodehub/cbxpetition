 ...
 * CBX Poll Widget
 */
class CBXPetitionSignaturesDisplayElemWidget extends \Elementor\Widget_Base {
	...
	public function get_name() {
		return 'cbxpetition_signatures_display';
	}//end method get_name
	...
	public function get_title() {
		return esc_html__( 'CBX Petition Signatures', 'cbxpetition' );
	...
	public function get_icon() {
		return 'cbxpetition-signatures-display-icon';
	}//end method get_icon
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'section_cbxpetition_signatures_display',
			[
				'label' => esc_html__( 'CBXPetition Signatures Display Widget Setting', 'cbxpetition' ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type'        => \Elementor\Controls_Manager::TEXT,
				'description' => esc_html__( 'Default signature listing limit', 'cbxpetition' ),
				'default'     => 20,
				'label_block' => true
			]
		);
		$this->add_control(
			'cbxpetition_order',
			[
				'label'       => esc_html__( 'Signatures Display Order', 'cbxpetition' ),
				'description' => esc_html__( 'Signatures Display Order By ASC/DESC', 'cbxpetition' ),
	...
	protected function render() {
		...
		$per_page = intval( $settings['cbxpetition_perpage'] );
		$order   = sanitize_text_field( $settings['cbxpetition_order'] );
		...
		if ( absint( $id ) <= 0 && ( false !== get_post_status( $id ) ) ) {
			...
			echo do_shortcode( '[cbxpetition_signatures petition_id="' . $id . '" perpage="' . $per_page . '" order="' . $order . '"]' );
		}
...
}//end method CBXPetitionSignaturesDisplayElemWidget
