...
if ( ! class_exists( 'CBXPetitionSignApproveUserEmail', false ) ) :

	/**
	 * Class CBXPetitionSignApproveUserEmail file
	 *
	 * Sending email alert to user when a sign is approved
	 */
	class CBXPetitionSignApproveUserEmail extends CBXPetitionEmail {
		/**
		 * Constructor.
		 */
		public function __construct() {
			$this->id          = 'sign_approve_user_email';
			$this->user_email  = true; //alert for user
			$this->title       = esc_html__( 'Signature approval email alert for user', 'cbxpetition' );
			$this->description = esc_html__( 'On signature approval email is sent to user.', 'cbxpetition' );
			...
			add_action( 'cbxpetition_sign_approved', [ $this, 'trigger' ], 10, 3 );
		 ...
		 * @since 2.0.0
		 */
		public function trigger( $log_data, $old_status, $new_status ) {
			$this->object = $log_data;

			if ( $this->is_enabled() ) {
				$sign_status = PetitionHelper::getPetitionSignStates();
				$log_id      = absint( $log_data['id'] );
				$petition_id = intval( $log_data['petition_id'] );
		...
		public function get_default_subject() {
			return esc_html__( 'Your Signature Has Been Approved!', 'cbxpetition' );
		...
		public function get_default_heading() {
			return esc_html__( 'Your Signature Approved', 'cbxpetition' );
	...
	}//end class CBXPetitionSignApproveUserEmail
endif;

return new CBXPetitionSignApproveUserEmail();